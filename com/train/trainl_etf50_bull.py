...
class train_etf50_kline(object):
    ...
    def add_stock_index(self, df0, index_list=None):
        ...
        df0["slope30"] = self.stand(
            ta.LINEARREG_SLOPE(ta.MA(close, timeperiod=60 if period >= 15 else 30), timeperiod=3))
    ...
    def saveStage(self, df2):
        ...
        for i in range(period, len(df2)):
            ma, close, p_l, p_h, top, lower, std, kdjm, sarm, pow, powm, atrb, sd, s1 = (
                ...
                "ma,close,p_l,p_h,top,lower,std,kdjm,sarm,pow,powm,atrb,sd,s1".split(","))
            ...
            if status == 0 and powm in [2, -2]:
                ...
                if vol > 0:
                    ...
                    if self.order(df2.iloc[i], -1, 3, ini):
                        isRun = False
                        # 平仓后再开
                ...
                if vol == 0:
                    ...
                    elif (p_h + sd2) >= top and pow < (self.turnline - 0.25) and kdjm < 0:
                        mode, isRun = -2, True

                    elif (p_l - sd2) <= lower and pow < (self.turnline - 0.25) and kdjm > 0:
                        mode, isRun = 2, True
