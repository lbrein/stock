...
class train_etf50_kline(object):
    ...
    def stag_etf(self, codes):
        self.codes = codes
        self.uid = '%s_%s' % (codes[0].replace(".", "_"), str(self.baisLine))
        print(self.uid)
        ...
        #df[df['mode'] == df['mode'].shift(1)]['mode'] = 0
        df.loc[df[df['mode']==df['mode'].shift(1)].index, 'mode'] = 0
    ...
    def saveStage(self, df2):
        ...
        for i in range(period, len(df2)):
            ...
            if vol > 0 and pos * mode < 0 and abs(mode) == 4:
                # 先平仓，再开仓
                self.order(df2.iloc[i], -1, mode, vol)
                # 再开仓
