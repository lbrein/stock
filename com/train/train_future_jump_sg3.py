...
class train_future_jump_sg(object):
    ...
    def jump(self, row, df0, df1):
        ...
        powm = np.sign(jump) if opt0 and opt1 else 2 * np.sign(ss) if opt2 and opt22 else 0
    ...
    def total(self, dfs, period= 21):
        ...
        for conds in self.iterCond():
            ...
            Ne = self.BS[self.code]['nightEnd']


            df0 = copy.deepcopy(df01[(df01['time'] >='09:10:00') & (df01['time'] <'10:15:00')])

            df2 = df0.apply(lambda row: self.jump(row, df01, df1), axis=1)
            for key in self.pointColumns:  df0[key] = df2[key]
            df0 = df0[self.startDate:]
            ...
            if isOpen == 0:
                if powm != 0 and opt0:
                 ...
                 if sign * powm < 0 and keep >= self.keepDays:
                     doc = self.order(df0.iloc[i], 0, uid,  df0, isstop=int(powm))
                     isOpen = 0
                     if doc is not None:
                         docs.append(doc)
                         if opt0: # 反向开仓
                             isRun, isOpen = True, int(powm) * 3
