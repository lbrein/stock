...
class train_future_klineExpect(object):
    ...
    def detect(self, df0, df1, period=15, uid=''):
        ...
        if len(res) > 0:
            ...
            # 计算夏普指数
            sha = (res['income'].sum() / self.iniAmount - 0.05 * self.testDays / 365) / inr.std()
            return {
    ...
    def stageApply(self, df0, df1, period=15, uid=''):
        ...
        ma, p_l, p_h, top, lower, delta, std, wd1, wd2, wd2m = (df0[key] for key in
                                                           "ma,p_l,p_h,top,lower,delta,std,widthDelta,widthDelta2,wd2m".split(
                                                               ","))
