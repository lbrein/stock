...
class train_future_macd(train_future_singleExpect):
    ...
    def Pool(self):
        ...
        for rs in lists:
            ...
            print(rs)
            if self.isAll == 0 and codes[0] not in ['RB','CU','ZC','FG','SC','MA']:  continue
    ...
    pointColumns = ['pow', 'powm', 'trend', 'isout']

    def point(self, row, ktype=15):
        wd1, s1, s2, high, low, open, close, top, lower, p_h, p_l, atr, atr1 = (row[key] for key in
                                                                                "wd1,slope,slope60,high,low,open,close,top,lower,p_h,p_l,atr,atr1".split(
                                                                                    ","))
        ...
        isout = -1 if p_h > top else 1 if p_l < lower else 0
        # powm
        powm = -3 * np.sign(trend) if (atr > 3.2) and (np.sign(trend) * isout < 0)  else 2 * sign if (pow > TL) else sign if pow > PL else 0

        columns = self.pointColumns
        return pd.Series([pow, powm, trend, isout], index=columns)
    ...
    def total(self, dfs, dfs2=None, period=60):
        ...
        df0["slope60"] = self.stand(ta.LINEARREG_SLOPE(ta.MA(close, timeperiod=60), timeperiod=5))
        ...
        # SAR
        sar = ta.SAR(df0['high'], df0['low'], acceleration=0.05, maximum=0.2)
        ...
        for scale in self.scaleList:
            for conds in self.iterCond():
                ...
                if isCvs:
                    cs = []
                    bans = 'ma,open,close,high,low,p_l,p_h,top,lower,std,delta,volume,sard,rel_price,width,volm,'.split(
                        ',')
    ...
    def stageApply(self, df0, df1, period=15, uid=''):
        ...
        for i in range(period, len(df0)):
            ...
            ma, close, p_l, p_h, top, lower, std, kdjm, sarm, pow, powm, sd, isout, atr, atr1, atrb = (
                df0.ix[i, key] for key in
                "ma,close,p_l,p_h,top,lower,std,kdjm,sarm,pow,powm,sd,isout,atr,atr1,atrb".split(","))

            sd2 = (self.scaleDiff2 - sd) * std / 2  # std修正

            if status == 0 and powm in [2, -2] and isOpen not in [6, -6]:
                """
                   突变点处理 
                   将布林带策略切换为策略 
                """
                ...
                elif isOpen == 0 and bulltype ==0 :
                    isOpen, isRun = int(4 * status), True

            elif (sarm * status < 0) or (abs(isOpen) in [4, 5] and isOpen * sarm < 0) or abs(powm)==3:
                """
                    交叉点: sarm结束点 
                    1、结束突变状态，变为布林带处理
                    2、根据节点状况开平新仓，状态为6 
                ...
                elif isOpen != 0 and isOpen * sarm < 0:
                ...
                if isOpen == 0:
                    ...
                    elif ((p_h + sd2) >= top or (isout < 0 and (p_h - sd2) >= ma)) and pow < (pd2 - sd) and kdjm < 0:
                        isOpen, isRun = -2, True

                    elif ((p_l - sd2) <= lower or (isout > 0 and (p_l + sd2) <= ma)) and pow < (pd2 + sd) and kdjm > 0:
                        isOpen, isRun = 2, True
                ...
                # 平仓
                    ...
                    elif cond3 >= 0 and pow < (self.powline) and cond1:
                        isOpen, isstop = 0, 0
                        isRun = True
                    ...
                    # 超过2倍 atr，止损
                        ...
                        if self.preNode is not None:
                            ...
                            if (sign * (Pd-close) > 2.0 * atrb) and sign * kdjm < 0:
