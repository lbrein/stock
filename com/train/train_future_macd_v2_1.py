...
class train_future_macd(train_future_singleExpect):
    ...
    def point(self, row, ktype=15, period=15):
        wd, wd1, wds, s1, s2, high, low, open, close, top, lower, atr, atr1, tu, td, sd = (row[key] for key in
                                                                                           "width,wd1,wdstd,slope,slope60,high,low,open,close,top,lower,atr,atr1,tu,td,sd".split(
                                                                                               ","))
        ...
        powm = 2 * sign if opt1 else sign if (pow > PL or wdf > 1.0) and wdd > 0.5 else 0
    ...
    def total(self, dfs, dfs2=None, period=60):
        ...
        df0["slope60"] = self.stand(ta.LINEARREG_SLOPE(ta.MA(close, timeperiod=60), timeperiod=3))
        ...
        # SAR
        sar = ta.SAR(df0['high'], df0['low'], acceleration=0.035, maximum=0.2)
        ...
        # 唐奇安线
        df0['tu30'] = ta.MAX(df0['high'].shift(1), timeperiod=60)
        df0['td30'] = ta.MIN(df0['low'].shift(1), timeperiod=60)

        # 唐奇安线
        df0['tu60'] = ta.MAX(df0['high'].shift(1), timeperiod=60)
        df0['td60'] = ta.MIN(df0['low'].shift(1), timeperiod=60)
    ...
    def stageApply(self, df0, df1, period=15, uid=''):
        ...
        for i in range(period, len(df0)):
            ...
            if isOpen!=0 and str(date) in adjusts:
                self.adjustOrder(df0.iloc[i], date)


            if status == 0 and powm in [2, -2] and not (isOpen in [6, -6] and isOpen * (close - ma) < 0):
                """
                   突变点处理 
                   将布林带策略切换为策略 
                """
